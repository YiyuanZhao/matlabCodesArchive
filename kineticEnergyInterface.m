...
% while convergeCriteria
    ...
    switch workingMode
        case 1
            diff = calculateDifference(kPointsMesh(1:3, :)', exportData.fermiLevel, Hkin, targetHkin);
        case 2
            diff = calculateDifference(kPointsMesh(4, :)', fermiLevel, Hkin, targetHkin);
    end
    % hoppingParameter = someAlgorithmToUpdateHoppingParameter()
% end
...
switch workingMode    
    case 1       
        scatter3(kPointsMesh(1, :), kPointsMesh(2, :), real(Hkin) - exportData.fermiLevel, 1);
        figure();
        scatter3(kPointsMesh(1, :), kPointsMesh(2, :), real(diff), 1);
    case 2
        plot(kPointsMesh(4, :), real(Hkin) - fermiLevel, 'b');
        hold on;
        plot(kPointsMesh(4, :), real(targetHkin), 'r');
        hold off;
        figure();
        plot(kPointsMesh(4, :), real(diff));
end
...
function diff = calculateDifference(kpointsRecip, fermiLevel ,calcHkin, targetHkin)
diff = zeros(1, length(kpointsRecip));
        for loopIndex = 1: length(kpointsRecip)
            diff(loopIndex) = calcHkin(loopIndex) - fermiLevel - targetHkin(loopIndex);
        end
end